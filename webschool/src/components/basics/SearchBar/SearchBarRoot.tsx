...
export default function SearchBarRoot({htmlFor, children, className, spanClassName} : SearchBarRootPorps) {
    return (
        <label htmlFor={htmlFor} className={twMerge("md:min-w-[200px] md:w-full flex-1 relative px-2 py-1.5 rounded-md flex items-center gap-1 cursor-text bg-zinc-100 dark:bg-webschool-200 overflow-hidden transition-colors duration-150", className)}>
            {children}
